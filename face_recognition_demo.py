...
from landmarks_detector_eyes import LandmarksDetectorEyes
...
from common.python import monitors
from common.python.helpers import resolution
from common.python.images_capture import open_images_capture

from common.python.model_zoo.model_api.models import OutputTransform
...
def build_argparser():
    ...
    models.add_argument('-m_lme', type=Path, required=True,
                        help='Required. Path to an .xml file with Facial Landmarks Detection Eyes model.')
    # models.add_argument('-m_hpe', type=Path, required=True,
    #                     help='Required. Path to an .xml file with Head Pose Estimation model.')
    ...
    infer.add_argument('-d_lme', default='CPU', choices=DEVICE_KINDS,
                       help='Optional. Target device for Facial Landmarks Detection Eyes '
                            'model. Default value is CPU.')
    # infer.add_argument('-d_hpe', default='CPU', choices=DEVICE_KINDS,
    #                    help='Optional. Target device for Head Pose Estimation '
    #                         'model. Default value is CPU.')    
...
class FrameProcessor:
    ...
    def __init__(self, args):
        ...
        self.landmarks_detector_eyes = LandmarksDetectorEyes(core, args.m_lme)
        #self.head_pose_estimator = HeadPoseEstimator(core, args.m_hpe)
        ...
        self.landmarks_detector_eyes.deploy(args.d_lme, self.QUEUE_SIZE)
        #self.head_pose_estimator.deploy(args.d_hpe, self.QUEUE_SIZE)
    ...
    def process(self, frame):
        ...
        landmarks_eyes = self.landmarks_detector_eyes.infer((frame, rois))
        #head_poses = self.head_pose_estimator.infer((frame, rois))
        ...
        #return [rois, landmarks, landmarks_eyes, head_poses, face_identities]
        return [rois, landmarks, landmarks_eyes, face_identities]


def draw_detections(frame, frame_processor, detections, output_transform):
    size = frame.shape[:2]
    frame = output_transform.resize(frame)
    #for roi, landmarks, landmarks_eyes, head_poses, identity in zip(*detections):
    for roi, landmarks, landmarks_eyes, identity in zip(*detections):
        ...
        for point_eyes in landmarks_eyes:
            x = xmin + output_transform.scale(roi.size[0] * point_eyes[0])
            y = ymin + output_transform.scale(roi.size[1] * point_eyes[1])
            cv2.circle(frame, (int(x), int(y)), 1, (0, 0, 255), 2)
        
        # TODO : head_poses 화면에 표시하기
